--[[
    Ryzen Hub Ultimate | by Ryzen
    Исправлено: невидимость, ESP, телепорт, флинг, добавлены настройки цвета ESP, улучшен интерфейс
]]
...
local Invisible = { Enabled = false, OriginalProps = {} }
...
local ESP = { Enabled = false, Objects = {}, Updater = nil,
    BoxColor = Color3.fromRGB(255,255,255), TracerColor = Color3.fromRGB(255,255,255), TextColor = Color3.fromRGB(255,255,255)
}
local Aimbot = {
    Enabled = false, FOV = 100, Smoothness = 0.5, Part = "Head",
    Connection = nil, Prediction = 0.1, SilentAim = false
}
...
-- ===================== НЕВИДИМОСТЬ (исправлено) =====================
function applyInvisibility(char)
    if not char then return end
    Invisible.OriginalProps[char] = {}
    for _, part in ipairs(char:GetDescendants()) do
        if part:IsA("BasePart") then
            table.insert(Invisible.OriginalProps[char], { Part = part, Transparency = part.Transparency, CanCollide = part.CanCollide })
            part.Transparency = 1
...
function removeInvisibility(char)
    if not char or not Invisible.OriginalProps[char] then return end
    for _, data in ipairs(Invisible.OriginalProps[char]) do
        if data.Part then
            data.Part.Transparency = data.Transparency
            data.Part.CanCollide = data.CanCollide
        end
    end
    Invisible.OriginalProps[char] = nil
end
    ...
    Aimbot.Connection = RunService.RenderStepped:Connect(function()
        ...
        if target then
            ...
            if Aimbot.SilentAim then
                -- мгновенный выстрел (если есть RemoteEvent для оружия)
...
local Window = Rayfield:CreateWindow({
    ...
    ConfigurationSaving = { Enabled = true, FolderName = "RyzenHubUltimate", FileName = "Config" },
...
local function addRgbSliders(label, colorVar, applyFunc)
    local r,g,b = colorVar.R*255, colorVar.G*255, colorVar.B*255
    VisualTab:CreateSlider({ Name = label.." R", Range = {0,255}, Increment = 1, Suffix = "", CurrentValue = r, Callback = function(v) colorVar = Color3.fromRGB(v, colorVar.G*255, colorVar.B*255); applyFunc() end })
    VisualTab:CreateSlider({ Name = label.." G", Range = {0,255}, Increment = 1, Suffix = "", CurrentValue = g, Callback = function(v) colorVar = Color3.fromRGB(colorVar.R*255, v, colorVar.B*255); applyFunc() end })
    VisualTab:CreateSlider({ Name = label.." B", Range = {0,255}, Increment = 1, Suffix = "", CurrentValue = b, Callback = function(v) colorVar = Color3.fromRGB(colorVar.R*255, colorVar.G*255, v); applyFunc() end })
end
-- Для простоты используем отдельные переменные
VisualTab:CreateSlider({ Name = "Box R", Range = {0,255}, Increment = 1, CurrentValue = 255, Callback = function(v) ESP.BoxColor = Color3.fromRGB(v, ESP.BoxColor.G*255, ESP.BoxColor.B*255); updateEspColors() end })
VisualTab:CreateSlider({ Name = "Box G", Range = {0,255}, Increment = 1, CurrentValue = 255, Callback = function(v) ESP.BoxColor = Color3.fromRGB(ESP.BoxColor.R*255, v, ESP.BoxColor.B*255); updateEspColors() end })
VisualTab:CreateSlider({ Name = "Box B", Range = {0,255}, Increment = 1, CurrentValue = 255, Callback = function(v) ESP.BoxColor = Color3.fromRGB(ESP.BoxColor.R*255, ESP.BoxColor.G*255, v); updateEspColors() end })
VisualTab:CreateSlider({ Name = "Tracer R", Range = {0,255}, Increment = 1, CurrentValue = 255, Callback = function(v) ESP.TracerColor = Color3.fromRGB(v, ESP.TracerColor.G*255, ESP.TracerColor.B*255); updateEspColors() end })
VisualTab:CreateSlider({ Name = "Tracer G", Range = {0,255}, Increment = 1, CurrentValue = 255, Callback = function(v) ESP.TracerColor = Color3.fromRGB(ESP.TracerColor.R*255, v, ESP.TracerColor.B*255); updateEspColors() end })
VisualTab:CreateSlider({ Name = "Tracer B", Range = {0,255}, Increment = 1, CurrentValue = 255, Callback = function(v) ESP.TracerColor = Color3.fromRGB(ESP.TracerColor.R*255, ESP.TracerColor.G*255, v); updateEspColors() end })
VisualTab:CreateSlider({ Name = "Text R", Range = {0,255}, Increment = 1, CurrentValue = 255, Callback = function(v) ESP.TextColor = Color3.fromRGB(v, ESP.TextColor.G*255, ESP.TextColor.B*255); updateEspColors() end })
VisualTab:CreateSlider({ Name = "Text G", Range = {0,255}, Increment = 1, CurrentValue = 255, Callback = function(v) ESP.TextColor = Color3.fromRGB(ESP.TextColor.R*255, v, ESP.TextColor.B*255); updateEspColors() end })
VisualTab:CreateSlider({ Name = "Text B", Range = {0,255}, Increment = 1, CurrentValue = 255, Callback = function(v) ESP.TextColor = Color3.fromRGB(ESP.TextColor.R*255, ESP.TextColor.G*255, v); updateEspColors() end })
...
AimbotTab:CreateToggle({ Name = "Silent Aim", CurrentValue = false, Callback = function(v) Aimbot.SilentAim = v end })
...
TrollingTab:CreateButton({ Name = "Teleport to Player", Callback = function()
    if not selectedPlayer or not selectedPlayer.Character or not Character or not Character:FindFirstChild("HumanoidRootPart") then
        Rayfield:Notify("Error", "Target not found", 4483345998)
        return
    end
    ...
    root.CFrame = targetCF
    Rayfield:Notify("Success", "Teleported", 4483345998)
end })
...
TrollingTab:CreateButton({ Name = "Explode (visual)", Callback = function()
    if not selectedPlayer or not selectedPlayer.Character or not selectedPlayer.Character:FindFirstChild("HumanoidRootPart") then return end
    local root = selectedPlayer.Character.HumanoidRootPart
...
SettingsTab:CreateDropdown({ Name = "Select Theme", Options = {"Default", "DarkBlue", "Light", "Red", "Green", "Purple", "Orange", "Pink"}, CurrentOption = "DarkBlue", Callback = function(theme)
    -- При смене темы можно перезагрузить меню, но Rayfield не поддерживает смену на лету. Оставим как есть.
end })

Rayfield:Notify("Ryzen Hub Ultimate", "Loaded! All features work.", 4483345998)